...
function ValidateForm() {
	jQuery("#formFour").validate({
		...
		errorPlacement: function (error, element) {
			if (
				element.attr("name") == "SortCode1" ||
				element.attr("name") == "SortCode2" ||
				element.attr("name") == "SortCode3"
			)
...
function GetSortCode() {
	return (
		jQuery("#SortCode1").val() +
		jQuery("#SortCode2").val() +
		jQuery("#SortCode3").val()
	);
}
...
function RedirectToGoCardless(redirectUrl) {
	...
	if (redirectUrl) {
		...
		Cookies.set("wordpress_guardian_amount", amount, {
			path: "/",
			expires: date,
		});
		...
		Cookies.set("wordpress_guardian_signup", signup, {
			path: "/",
			expires: date,
		});
...
function zapier() {
	var data = {
		...
		giftAid: jQuery("#GiftAid").is(":checked"),
		comments:
			jQuery("#inspiration_question").val() + " " + jQuery("#Comment").val(),
