...
<div class="better-grid card-thirds hfj-block">
    ...
    <?php while (have_rows("third_cards")):
    	the_row(); ?>
        <?php
 	...
 	$video
 ) { ?>data-toggle="modal" data-target="#video-modal-cards-thirds" data-src="<?php echo $video; ?>" <?php } else {if (
 		$target
 	) {
 		echo 'target="' . $link_target . '"';
 	} ?> <?php if (
 	!is_admin()
 ) { ?>href="<?php echo $link_url; ?>" <?php } ?> <?php } ?> class="<?php if ($dropdown) {
