...
<div class="better-grid grid-block <?php echo $class_name; ?>">
    <!-- inner grid -->
    <div class="better-grid grid-block__inner grid-block__inner--row-space-<?php echo $row_space; ?> <?php if (
 	$indent
 ) {
 	echo "grid-block__inner--indent";
 } ?>">
        <!-- inner blocks to only allow grid-content -->
        <?php
        $template = [["acf/grid-container"], ["acf/grid-container"]];
        $allowed_blocks = ["acf/grid-container"];
