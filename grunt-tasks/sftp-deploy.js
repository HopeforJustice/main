...
module.exports = function (deployBase, themeName) {
	return {
		...
		src: deployBase,
		dest: 'wp-content',
		exclusions: [deployBase + '/uploads', deployBase + '/languages', deployBase + '/plugins', deployBase + '/upgrade', deployBase + '/plugins', '.ftppass', '.git', '.gitignore', 'node_modules', '.sass-cache', 'npm-debug.log', 'debug.log', '.DS_Store', '.sftpcache.json'],
		serverSep: '/',
		concurrency: 4,
		progress: true
		},
